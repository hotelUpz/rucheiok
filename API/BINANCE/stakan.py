...
#         raise RuntimeError(f"Binance request failed: path={path} params={params} err={last_err}")
...
#     @classmethod
#     def _valid_limit(cls, depth: int) -> int:
#         target = max(1, int(depth))
#         for value in cls.VALID_LIMITS:
#             if value >= target:
#                 return value
#         return cls.VALID_LIMITS[-1]

#     async def fetch_depth(self, symbol: str, depth: int = 7) -> OrderBookSnapshot:
#         request_limit = self._valid_limit(depth)
#         data = await self._get_json(
#             "/fapi/v1/depth",
#             params={"symbol": symbol.upper().strip(), "limit": request_limit},
#         )
#         bids = self._parse_levels(data.get("bids", []))[:depth]
#         asks = self._parse_levels(data.get("asks", []))[:depth]
#         receipt_ms = int(time.time() * 1000)
#         remote_ms = int(data.get("E") or data.get("T") or 0)
...
#             remote_event_time_ms=remote_ms or None,
#         )
