...
#         raise RuntimeError(f"Phemex request failed: path={path} params={params} err={last_err}")
...
#     @staticmethod
#     def _extract_book(data: Any) -> Dict[str, Any]:
#         if not isinstance(data, dict):
#             raise RuntimeError("Bad Phemex orderbook payload")
#         result = data.get("result") if isinstance(data.get("result"), dict) else data
#         if not isinstance(result, dict):
#             raise RuntimeError("Bad Phemex orderbook result")
#         if isinstance(result.get("orderbook_p"), dict):
#             return {
#                 "bids": result["orderbook_p"].get("bids") or [],
#                 "asks": result["orderbook_p"].get("asks") or [],
#                 "timestamp": result.get("timestamp") or 0,
#             }
#         if isinstance(result.get("book"), dict):
#             return {
#                 "bids": result["book"].get("bids") or [],
#                 "asks": result["book"].get("asks") or [],
#                 "timestamp": result.get("timestamp") or 0,
#             }
#         return {
#             "bids": result.get("bids") or [],
#             "asks": result.get("asks") or [],
#             "timestamp": result.get("timestamp") or result.get("time") or 0,
#         }

#     async def fetch_depth(self, symbol: str, depth: int = 7) -> OrderBookSnapshot:
#         sym = symbol.upper().strip()
#         data = await self._get_json("/md/v2/orderbook", params={"symbol": sym})
#         book = self._extract_book(data)
#         bids = self._parse_levels(book.get("bids") or [])[:depth]
#         asks = self._parse_levels(book.get("asks") or [])[:depth]
#         receipt_ms = int(time.time() * 1000)
#         remote_ts = int(book.get("timestamp") or 0)
#         if remote_ts > 10_000_000_000_000:
#             remote_ts //= 1_000_000
...
#             remote_event_time_ms=remote_ts or None,
#         )
